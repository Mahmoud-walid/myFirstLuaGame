...
tove.init = function(path)
    ...
    local lib = ffi.load(love.filesystem.getSourceBaseDirectory() ..
                             "/lib/tove/" .. libName[love.system.getOS()])
    tove.lib = lib
    tove.getVersion = function() return ffi.string(lib.GetVersion()) end
    ...
    do
        ...
        for i, n in ipairs(levels) do ilevels[n] = i end
        ...
        tove.getReportLevel = function() return reportLevel end
        ...
        tove.report = report
        tove.warn = function(s) report(s, lib.TOVE_REPORT_WARN) end
        tove.slow = function(s) report(s, lib.TOVE_REPORT_SLOW) end
    ...
    do
        ...
        if string.find(device, "Parallels") ~= nil then env.matrows = 4 end
    end

    env.matsize = ffi.sizeof("float[?]", 3 * env.matrows)

    lib.ConfigureShaderCode(env.graphics.glsl3 and lib.TOVE_GLSL3 or
                                lib.TOVE_GLSL2, env.matrows)
    ...
    tove.transformed = function(source, ...)
        ...
        return setmetatable({s = source, args = {a, b, c, d, e, f}},
                            tove.Transform)
    end
    ...
    local function findEnum(enums, value)
        for k, v in pairs(enums) do if v == value then return k end end
    ...
    tove._str = function(name) return ffi.string(lib.NameCStr(name)) end
    ...
    local Paint = (function()
        ...
        function Paint:getType()
            ...
            for name, enum in pairs(paintTypes) do
                if t == enum then return name end
            end
        end

        --- Get number of colors.

        function Paint:getNumColors() return lib.PaintGetNumColors(self) end
        ...
        function Paint:clone() return lib.ClonePaint(self) end
        ...
        local function importGradientColors(g, colors)
            for _, c in ipairs(colors) do g:addColorStop(unpack(c)) end
        end
        ...
        Paint._wrap = function(r, g, b, a)
            if ffi.istype("TovePaintRef", r) then return r end
            local t = type(r)
            if t == "number" then
                return newColor(r, g, b, a)
            elseif t == "string" and r:sub(1, 1) == '#' then
                r = r:gsub("#", "")
                return newColor(tonumber("0x" .. r:sub(1, 2)) / 255,
    ...
    local newCommand = (function()
        ...
        Command.__newindex = function(self, key, value)
            lib.SubpathSetCommandValue(rawget(self, "_t"), rawget(self, "_c"),
                                       _attributes[key], value)
        end

        function Command:commit() lib.SubpathCommit(self._t) end
    ...
    local Subpath = (function()
        ...
        Points.__index = function(self, i)
            if i == "count" then
                ...
                return setmetatable({traj = self.traj, i = self.i0 + i - 1},
                                    Point)
            end
        end
        ...
        Curve.__index = function(self, key)
            if key == "p" then
                return setmetatable({traj = self.traj, i0 = self.curve * 4},
                                    Points)
            end
            local i = _attributes[key]
            if i == nil then return nil end
            return lib.SubpathGetCurveValue(self.traj, self.curve, i)
        end
        Curve.__newindex = function(self, key, value)
            local i = _attributes[key]
            if i == nil then return nil end
            return lib.SubpathSetCurveValue(self.traj, self.curve, i, value)
        end
        ...
        function Curve:makeSmooth(a, dir)
            return lib.SubpathMakeSmooth(self.traj, self.curve, dir or 0,
                                         a or 0.5)
        end
        ...
        Curves.__index = function(self, curve)
            if curve == "count" then
                ...
                return
                    setmetatable({traj = self.traj, curve = curve - 1}, Curve)
            end
        end
        ...
        function Subpath:move(k, x, y, h)
            lib.SubpathMove(self, k, x, y,
                            h and handles[h] or lib.TOVE_HANDLE_FREE)
        end
        ...
        function Subpath:drawRect(x, y, w, h, rx, ry)
            return newCommand(self, lib.SubpathDrawRect(self, x, y, w, h or w,
                                                        rx or 0, ry or 0))
        end
        ...
        function Subpath:drawCircle(x, y, r)
            return newCommand(self, lib.SubpathDrawEllipse(self, x, y, r, r))
        end

        --- Draw an ellipse.

        function Subpath:drawEllipse(x, y, rx, ry)
            return newCommand(self,
                              lib.SubpathDrawEllipse(self, x, y, rx, ry or rx))
        end
        ...
        function Subpath:set(arg)
            if getmetatable(arg) == tove.Transform then
                lib.SubpathSet(self, arg.s, unpack(arg.args))
            else
                lib.SubpathSet(self, arg, 1, 0, 0, 0, 1, 0)
            end
        end
        ...
        function Subpath:invert() lib.SubpathInvert(self) end
        ...
        function Subpath:warp(f)
            ...
            for i = 0, n - 1, 3 do
                ...
                p[2 * i + 0] = x
                p[2 * i + 1] = y
                if curvature ~= nil then
                    c[j].curvature = curvature
                end
                j = j + 1
            end
        ...
        tove.newSubpath = function(...)
            ...
            if #{...} > 0 then self:setPoints(...) end
            return self
        end

        --- Clone.

        function Subpath:clone() return lib.CloneSubpath(self) end

        function Subpath:serialize()
            local n = lib.SubpathGetNumPoints(self)
            local p = lib.SubpathGetPointsPtr(self)
            local q = {}
            for i = 1, n * 2 do q[i] = p[i - 1] end
            return {points = q, closed = self.isClosed}
        end
    ...
    local Path = (function()
        ...
        Subpaths.__index = function(self, i)
            if i == "count" then
                ...
                return ffi.gc(lib.PathGetSubpath(self.path, i),
                              lib.ReleaseSubpath)
            end
        end
        ...
        function Path:addSubpath(t) lib.PathAddSubpath(self, t) end
        ...
        function Path:animate(a, b, t) lib.PathAnimate(self, a, b, t) end

        --- Warp points.

        function Path:warp(f)
            local subpaths = self.subpaths
            for i = 1, subpaths.count do subpaths[i]:warp(f) end
        end
        ...
        function Path:nearest(x, y, max, min)
            ...
            for i = 1, n do
                ...
                if t >= 0 then return d, t, traj end
            end
            return false
        end
        ...
        function Path:set(arg, swl)
            if getmetatable(arg) == tove.Transform then
                lib.PathSet(self, arg.s, swl or false, unpack(arg.args))
            else
                lib.PathSet(self, arg, false, 1, 0, 0, 0, 1, 0)
            end
        end
        ...
        tove.newPath = function(data)
            local t = type(data)
            local p = ffi.gc(lib.NewPath(t == "string" and data or nil),
                             lib.ReleasePath)
            if t == "table" then p:deserialize(data) end
            return p
        end
    ...
    local _mesh = (function()
        ...
        function AbstractMesh:getUsage(what) return self._usage[what] end

        function AbstractMesh:updateVertices()
            local mesh = self._mesh
            if mesh ~= nil then
                local vdata = self._vdata
                lib.MeshSetVertexBuffer(self._tovemesh, vdata:getPointer(),
                                        vdata:getSize())
        ...
        function AbstractMesh:updateTriangles()
            ...
            if mesh ~= nil then
                ...
                if indexCount < 1 then return end
                ...
                lib.MeshCopyIndexData(self._tovemesh, idata:getPointer(),
                                      idata:getSize())
        ...
        function AbstractMesh:getMesh()
            ...
            if ni < 1 then return nil end

            local mesh = love.graphics.newMesh(self._attributes, n,
                                               getTrianglesMode(self._tovemesh),
                                               self:getMeshUsage())
            self._mesh = mesh
        ...
        PaintMesh._attributes = {
            {"VertexPosition", "float", 2}, {"VertexPaint", "byte", 4}
        }
        ...
        function ColorMesh:retesselate(flags)
            ...
            if bit.band(updated,
                        lib.UPDATE_MESH_VERTICES + lib.UPDATE_MESH_COLORS) ~= 0 then
                self:updateVertices()
            end
    ...
    local _shaders = (function()
        ...
        local function createSendSync(shader, code, sync)
            ...
            for i = 0, 1 do
                local k = embedded[i]
                if k ~= 0 then table.insert(sent, tove._sent[k]) end
            end
            if #sent == 0 then
                return sync
            else
        ...
        local function newGeometryFillShader(data, sync, fragLine, meshBand,
                                             debug)
            local geometry = data.geometry

            local code = lib.GetGPUXFillShaderCode(data, fragLine, meshBand,
                                                   debug)

            local shader = lg.newShader(ffi.string(code.code))

            shader:send("constants", {
                geometry.maxCurves, geometry.listsTextureSize[0],
                geometry.listsTextureSize[1]
            })
        ...
        local function newGeometryLineShader(data, sync)
            ...
            if style < 1 then return nil end
        ...
        local function newPaintShader(numPaints, numGradients)
            return lg.newShader(ffi.string(
                                    lib.GetPaintShaderCode(numPaints,
                                                           numGradients)))
        end
        ...
        local feed = (function()
            ...
            function NullColorFeed:beginInit() end
            function NullColorFeed:endInit(path) end
            function NullColorFeed:updateUniforms(chg1, path) end
            ...
            function MeshBands:initData()
                ...
                for i = 1, 3 do
                    local bandDataSize =
                        ffi.sizeof("float[?]", data.maxBandsVertices *
                                       MeshBands.FLOATS_PER_VERTEX[i])
            ...
            function MeshBands:createMeshes(fillShader)
                ...
                }, data.maxBandsVertices, "triangles")
            ...
            local function bandsGetVertexView(self, i)
                ...
                if view == nil and size > 0 then
                    view = love.data.newDataView(self.bandsVertexByteData[i], 0,
                                                 ffi.sizeof("float[?]", size *
                                                                MeshBands.FLOATS_PER_VERTEX[i]))
                    cache[size] = view
                end
                return view, size
            ...
            function GeometrySend:warmup()
                if not self._warmup then
                    ...
                    if self.lineShader ~= nil and self.lineShader ~=
                        self.fillShader then
            ...
            function GeometrySend:beginInit()
                ...
                self.boundsByteData = love.data.newByteData(ffi.sizeof(
                                                                "ToveBounds"))
                data.bounds = self.boundsByteData:getPointer()

                local listsImageData = love.image.newImageData(
                                           data.listsTextureSize[0],
                                           data.listsTextureSize[1],
                                           ffi.string(data.listsTextureFormat))
                data.listsTextureRowBytes =
                    listsImageData:getSize() / data.listsTextureSize[1]

                local curvesImageData = love.image.newImageData(
                                            data.curvesTextureSize[0],
                                            data.curvesTextureSize[1],
                                            ffi.string(data.curvesTextureFormat))
                data.curvesTextureRowBytes =
                    curvesImageData:getSize() / data.curvesTextureSize[1]
                ...
                self.lookupTableMetaByteData =
                    love.data.newByteData(ffi.sizeof("ToveLookupTableMeta"))
                data.lookupTableMeta = self.lookupTableMetaByteData:getPointer()

                if self.bands ~= nil then self.bands:initData() end
            end
            ...
            local function makeDrawLine(feed)
                ...
                local drawLine = function(...)
                    ...
                    if lineRuns == nil then return end
                ...
                local drawTransparentLine = function(...)
                    local border = geometry.strokeWidth + geometry.miterLimit *
                                       2
                    lg.stencil(drawLine, "replace", 1)
                    lg.setStencilTest("greater", 0)
                    local x0, y0 = bounds.x0, bounds.y0
                    lineShader:send("draw_mode", 2)
                    lg.rectangle("fill", x0 - border, y0 - border,
            ...
            function GeometrySend:endInit(lineStyle)
                ...
                local curvesTexture = love.graphics.newImage(
                                          self.curvesImageData)
        ...
        local function newMeshFeedData(name, graphics, tess, usage, resolution)
            ...
            local link = ffi.gc(lib.NewColorFeed(graphics._ref, resolution),
                                lib.ReleaseFeed)

            local alloc = lib.FeedGetColorAllocation(link)
            local matrixData = love.data.newByteData(
                                   (1 + alloc.numGradients) * env.matsize)
            local imageData = love.image.newImageData(alloc.numPaints,
                                                      alloc.numColors, "rgba8")
            ...
            gradientData.colorsTextureRowBytes =
                imageData:getSize() / alloc.numColors
        ...
        local newMeshShader = function(name, graphics, tess, usage, resolution)
            return setmetatable({
                ...
                linkdata = newMeshFeedData(name, graphics, tess, usage,
                                           resolution)
            }, MeshShader)
        end

        function MeshShader:getMesh() return self.linkdata.mesh end

        function MeshShader:recreate()
            tove.warn("full mesh recreation triggered in " ..
                          tove._str(self._name))
            self.linkdata = newMeshFeedData(self.name, self.graphics, self.tess,
                                            self.usage, self.resolution)
        end
        ...
        function MeshShader:update()
            ...
            local flags = graphics:fetchChanges(
                              lib.CHANGED_POINTS + lib.CHANGED_GEOMETRY)
            ...
            elseif bit.band(flags, lib.CHANGED_POINTS) ~= 0 then
                if self.usage["points"] == "static" then
                    tove.slow("static mesh points changed in " ..
                                  tove._str(self._name))
                    self:recreate()
                    return
                end
                ...
                if self.usage["triangles"] ~= "static" then
                    tessFlags = bit.bor(tessFlags,
                                        lib.UPDATE_MESH_AUTO_TRIANGLES)
                end
        ...
        local function newComputeFeedData(path, quality, debug)
            ...
            local link = ffi.gc(lib.NewGeometryFeed(path, fragLine),
                                lib.ReleaseFeed)
            ...
            if fragLine or data.color.fill.style > 0 then
                fillShader, syncShader =
                    newGeometryFillShader(data, nil, fragLine, meshBand,
                                          debug or false)
            end
            ...
            local lineColorSend = feed.newColorSend(lineShader, "line",
                                                    data.color.line)
            lineColorSend:beginInit()
            local fillColorSend = feed.newColorSend(fillShader, "fill",
                                                    data.color.fill)
            fillColorSend:beginInit()

            local geometryFeed = feed.newGeometrySend(fillShader, lineShader,
                                                      data.geometry, meshBand)
        ...
        function ComputeShader:debug(curve)
            if curve == "off" then
                self.linkdata = newComputeFeedData(self.path,
                                                   self.linkdata.quality, false)
                self.debugging = nil
                return
            end
    ...
    local Graphics = (function()
        ...
        local createTesselator = (function()
            ...
            tove.newAdaptiveTesselator =
                function(resolution, recursionLimit)
                    return ffi.gc(lib.NewAdaptiveTesselator(resolution or 128,
                                                            recursionLimit or 8),
                                  lib.ReleaseTesselator)
                end

            tove.newRigidTesselator = function(subdivisions)
                return ffi.gc(lib.NewRigidTesselator(subdivisions),
                              lib.ReleaseTesselator)
            end
        ...
        local function gcpath(p) return p and ffi.gc(p, lib.ReleasePath) end
        ...
        tove.newGraphics = function(data, size)
            ...
            local graphics = setmetatable({
                ...
                paths = setmetatable({_ref = ref}, Paths)
            }, Graphics)
        ...
        --- Remove all @{Path}s.
        function Graphics:clear() lib.GraphicsClear(self._ref) end

        local function attachTesselator(display, usage)
            if display.mode == "mesh" then
                display.tesselator = createTesselator(usage,
                                                      unpack(display.quality))
            end
            return display
        end
        ...
        local function makeDisplay(mode, quality, usage)
            ...
            return attachTesselator({
                mode = mode,
                quality = clonedQuality,
                tesselator = nil
            }, usage)
        end

        --- Set name.
        function Graphics:setName(name) lib.NameSet(self._name, name) end
        ...
        function Graphics:clone()
            ...
            for k, v in pairs(self._usage) do g._usage[k] = v end
            return g
        end
        ...
        function Graphics:beginSubpath()
            return ffi.gc(lib.GraphicsBeginSubpath(self._ref),
                          lib.ReleaseSubpath)
        end
        ...
        function Graphics:arc(x, y, r, startAngle, endAngle, ccw)
            lib.SubpathArc(self:beginSubpath(), x, y, r, startAngle, endAngle,
                           ccw or false)
        end
        ...
        function Graphics:drawRect(x, y, w, h, rx, ry)
            local t = self:beginSubpath()
            return newCommand(t, lib.SubpathDrawRect(t, x, y, w, h or w,
                                                     rx or 0, ry or 0))
        end
        ...
        function Graphics:getUsage() return self._usage end
        ...
        function Graphics:clearCache() self._cache = nil end
        ...
        function Graphics:set(arg, swl)
            if getmetatable(arg) == tove.Transform then
                lib.GraphicsSet(self._ref, arg.s._ref, swl or false,
                                unpack(arg.args))
            else
                lib.GraphicsSet(self._ref, arg._ref, false, 1, 0, 0, 0, 1, 0)
            end
        end
        ...
        function Graphics:rescale(size, center)
            ...
            if math.abs(r) > 1e-8 then
                ...
                if center or true then
                    self:set(tove.transformed(self, 0, 0, 0, s, s,
                                              (x0 + x1) / 2, (y0 + y1) / 2),
                             true)
                else
        ...
        function Graphics:rasterize(width, height, tx, ty, scale, settings)
            if width == "default" then
                ...
                return self:rasterize(resolution * (x1 - x0),
                                      resolution * (y1 - y0), -x0 * resolution,
                                      -y0 * resolution, resolution, settings),
            ...
            lib.GraphicsRasterize(self._ref, imageData:getPointer(), width,
                                  height, stride, tx or 0, ty or 0, scale or 1,
                                  settings)
        ...
        function Graphics:warp(f)
            ...
            for i = 1, paths.count do paths[i]:warp(f) end
        end
        ...
        function Graphics:serialize()
            local paths = self.paths
            local p = {}
            for i = 1, paths.count do p[i] = paths[i]:serialize() end
            local d = self._display
            return {
                version = 1,
                paths = p,
        ...
        local create = (function()
            ...
            local function createDrawMesh(mesh, x0, y0, s)
                if mesh == nil then
                    return function(x, y, r, sx, sy) end
                else
            ...
            local function _updateFlatMesh(graphics)
                ...
                if bit.band(flags,
                            lib.CHANGED_FILL_STYLE + lib.CHANGED_LINE_STYLE) ~=
                    0 then
            ...
            create.texture = function(self)
                ...
                if mode == "fast" then
                    lib.SetRasterizeSettings(settings, "fast", lib.NoPalette(),
                                             1, 0, nil, 0)
                elseif (mode or "best") == "best" then
                    ...
                    if type(noise) == "table" then
                        ...
                        noiseDataSize = math.floor(
                                            math.sqrt(data:getSize() /
                                                          ffi.sizeof("float")))
                    end
                    if not lib.SetRasterizeSettings(settings, algo or "jarvis",
                                                    palette or lib.NoPalette(),
                                                    spread or 1, noise or 0.01,
                                                    noiseData, noiseDataSize) then
                        error(table.concat({
                            "illegal rasterize settings:", unpack(quality)
                        }, " "))
                    end
                elseif mode == "retro" then
                    ...
                    elseif type(ipal) == "table" then
                        ...
                        palette = ffi.gc(lib.NewPalette(colors, size),
                                         lib.ReleasePalette)
                    else
                        palette = ipal
                    end

                    if not lib.SetRasterizeSettings(settings, algo or "stucki",
                                                    palette, spread or 1,
                                                    noise or 0, nil, 0) then
                        error(table.concat({
                            "illegal rasterize settings:", unpack(quality)
                        }, " "))
                    end
                ...
                local imageData, x0, y0, x1, y1, resolution = self:rasterize(
                                                                  "default",
                                                                  settings)
            ...
            create.mesh = function(self)
                ...
                if lib.GraphicsAreColorsSolid(self._ref) or usage["shaders"] ==
                    "avoid" then
                    ...
                    local shader = _shaders.newMeshShader(name, self, tess,
                                                          usage, 1)
    ...
    local Shape = (function()
        ...
        function Container:removeChildAt(...)
            ...
            for _, i in ipairs(j) do table.remove(self._children, i) end
        end
        ...
        tove.newStage = function() return tove.newContainer() end
        ...
        function Shape:animate(t)
            ...
            for _, child in ipairs(self.children) do child:animate(t) end
        end
        ...
        function Shape:setDisplay(...) self.graphics:setDisplay(...) end
    ...
    local Animation = (function()
        ...
        local function _linear(x) return x end
        ...
        function Tween:to(graphics, duration, ease)
            table.insert(self._to, {
                graphics = graphics,
                duration = duration,
                ease = ease or _linear
            })
        ...
        local function morphify(graphics)
            ...
            for i, g in ipairs(graphics) do refs[i - 1] = g._ref end
        ...
        tove.newFlipbook = function(fps, tween, ...)
            display = {...}
            if next(display) == nil then display = {"texture"} end
            local frames = {}
        ...
        Flipbook.__newindex = function(self, key, value)
            if key == "t" then
                ...
                self._i = math.min(
                              math.max(1, 1 + math.floor(value * self._fps)),
                              #self._frames)
            end
        end

        --- Draw.
        function Flipbook:draw(...) self._frames[self._i]:draw(...) end
        ...
        Animation.__newindex = function(self, key, value)
            if key == "t" then
                ...
                if n > 1 then
                    ...
                    local f0 = f[lo]
                    local f1 = f[lo + 1]
                    self._graphics:animate(f0.graphics, f1.graphics, f1.ease(
                                               (t - f0.offset) / f1.duration))
                end
            end
        end
        ...
        function Animation:draw(...) self._graphics:draw(...) end

        function Animation:debug(...) self._graphics:debug(...) end

        function Animation:setCacheSize(s) self._graphics:setCacheSize(s) end

        function Animation:setName(n) self._graphics:setName(n) end
    end)()
end
